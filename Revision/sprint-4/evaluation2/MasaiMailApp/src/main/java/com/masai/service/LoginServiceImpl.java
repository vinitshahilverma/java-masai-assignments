...
import com.masai.repositrory.CurrentUserSessionRepo;
...
public class LoginServiceImpl implements LoginService {
	...
	public String loginAccount(Login login) throws LoginException {
		...
		User user = uRepo.findByEmail(login.getUserName());

		if (user != null) {

			if (user.getPassword().equals(login.getPassword())) {

				CurrentUserSession curr = sRepo.findByEmail(login.getUserName());
				...
				if (curr != null) {
					...
					curr1.setEmail(login.getUserName());
